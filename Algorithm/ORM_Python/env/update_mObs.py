...
def update_mObs(movingObs, dt):
    ...
    for i in range(0,countObs): 
        if movingObs[i,7] == 0: # movingObs 2x8
            ...
            posObs = posObs + vObs*dt*np.array([np.cos(dirObs),np.sin(dirObs)])
            if disObs >= lenObs:
                dirObs = limit_angle(dirObs + np.pi)
                disObs = 0
            movingObs[i, :] = [posObs[0], posObs[1], disObs, radObs, dirObs, lenObs, vObs, 0]
            ...
            posObs = centerCir + radCir*np.array([np.cos(angleObs),np.sin(angleObs)])
            movingObs[i, :] = [posObs[0], posObs[1], radCir, radObs, centerCir, vObs, 1]
        else:
            posObs = movingObs[i, 0:2]
            radCir = movingObs[i, 2]
            radObs = movingObs[i, 3]
